...
for file in files:
    ...
    gt_file = os.path.join(ge_path, '{}.txt'.format(stem))
    ...
    re_img = cv.resize(img, None, None, fx=img_scale, fy=img_scale, interpolation=cv.INTER_LINEAR)
    re_size = re_img.shape
    cv.imwrite(os.path.join(out_path, stem) + '.jpg', re_img)
    ...
    for line in lines:
        ...
        pt_x = np.zeros((4, 1))
        pt_y = np.zeros((4, 1))
        pt_x[0, 0] = int(float(split_line[0]) / img_size[1] * re_size[1])
        pt_y[0, 0] = int(float(split_line[1]) / img_size[0] * re_size[0])
        pt_x[1, 0] = int(float(split_line[2]) / img_size[1] * re_size[1])
        pt_y[1, 0] = int(float(split_line[3]) / img_size[0] * re_size[0])

        pt_x[2, 0] = int(float(split_line[4]) / img_size[1] * re_size[1])
        pt_y[2, 0] = int(float(split_line[5]) / img_size[0] * re_size[0])
        pt_x[3, 0] = int(float(split_line[6]) / img_size[1] * re_size[1])
        pt_y[3, 0] = int(float(split_line[7]) / img_size[0] * re_size[0])
        ...
        with open(os.path.join('label_tmp', stem) + '.txt', 'a') as f:
            for i in range(len(x_left)):
                f.writelines('text\t')
                f.writelines('{}\t'.format(str(int(x_left[i]))))
                f.writelines('{}\t'.format(str(int(ymin))))
                f.writelines('{}\t'.format(str(int(x_right[0]))))
                f.writelines('{}\n'.format(str(int(ymax))))
