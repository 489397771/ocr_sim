...
def generate_xml(name, lines, img_size, class_sets, doncateothers=True):
    ...
    for line in lines:
        ...
        occlusion = int(0)
        x1 = int(float(split_line[1]) + 1)
        y1 = int(float(split_line[2]) + 1)
        x2 = int(float(split_line[3]) + 1)
        y2 = int(float(split_line[4]) + 1)
        truncation = float(0)
...
if __name__ == '__main__':
    ...
    for dset in ['train']:
        ...
        class_set_dict = dict((k, i) for i, k in enumerate(class_sets))
        allclasses = {}
        ...
        for file in files:
            ...
            with open(file, 'r') as f:
                lines = f.readlines()
            img_file = os.path.join(_imagedir, '{}.jpg'.format(stem))
            print(img_file)
            ...
            cv.imwrite(os.path.join(_dest_img_dir, '{}.jpg'.format(stem)), img)
            ...
            for obj in objs:
                cls = obj['class']
                allclasses[cls] = 0 if not cls in list(allclasses.keys()) else allclasses[cls] + 1

            for cls in cls_in_image:
                if cls in class_sets:
                    fs[class_set_dict[cls]].writelines(stem + ' 1\n')
